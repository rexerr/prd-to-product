...
> **Superseded in part by [D-044](DECISIONS.md) (2026-06-19).** `design-system-bootstrap` now **has** an adopt mode that automates the three product-side steps below — copy the rendered design into `design/reference/`, copy the bundle's tokens into the product CSS, and emit the import rule. D-044 narrows [D-008](DECISIONS.md), which is why the "the kit builds no adopt-mode" / "not in this kit" / "no `prd-to-product` skill is built for this" claims in this doc are no longer literally true. **This doc stands as the manual fallback** — the playbook for product-side cases where the skill isn't run, plus the fidelity findings and the Rule-of-Two bundle ledger that still govern. The import rule below is the source of record the skill's `templates/design-adoption.md.template` copies from.
