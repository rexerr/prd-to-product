<!-- Synced through: C-41 (steinberger) · CF-29/16-keepers (pocock-craft) · DG-02 (designer) · AB-03/AB-i3 (claude-skill-bundles) — 2026-06-18; CF-03 adopted (→D-023) 2026-06-17; C-09 + CF-06 adopted (→D-024) 2026-06-18; C-14 adopted (→D-025) 2026-06-18; C-10 adopted (→D-026) 2026-06-18; CF-04 adopted (→D-027) 2026-06-18; C-15 adopted (→D-028) 2026-06-18; CF-02 adopted (→D-029) 2026-06-18; DG-02 declined-and-parked (→D-030) 2026-06-18 — Wave 1 cribs all resolved. gstack mined + adversarially verified 2026-06-18 (G-01–G-21; 7 survivors → Wave 2). Wave-2 Sprint 1 underway: C-16 adopted (→D-031) 2026-06-18; G-16 adopted (→D-032) + G-01 adopted-as-revised (→D-032) 2026-06-18; CF-15 deferred from Sprint 1 (near-no-op on furnace-plan; stays Proposed); CF-20 adopted (→D-033) 2026-06-18; CF-23 adopted (→D-034, mirrored to DECISIONS_ACTIVE) 2026-06-18; G-11 adopted (→D-035) 2026-06-18 — Sprint 1 complete (C-16/G-16/G-01/CF-20/CF-23/G-11 adopted, CF-15 deferred). Wave-2 Sprint 2 complete (prd-creator surface): CF-18 adopted (→D-036, rule 1 scoped) + G-06 adopted (→D-037) + G-08 adopted (→D-038) 2026-06-18. Wave-2 Sprint 3 (context-engineering surface) underway: C-09/CF-06/C-14 retro-template backport into the scaffolded template + `output-small` fixture (→D-039) 2026-06-18; DG-01 even-coverage synthesis rule adopted as a conditional modular `.claude/rules` scaffold, Q35a-gated (→D-040) 2026-06-18; CF-13 fire-wired-behavior bullet adopted into the scaffolded verification rule, both shapes + `output-small`, furnace-plan landing deferred as near-covered (→D-041) 2026-06-18; AB-03 per-project-type red-capable-repro table adopted into the scaffolded repro rule, both shapes + `output-small`, this-repo CLAUDE.md excluded (→D-045) 2026-06-19; G-14 generated-file provenance banner adopted across all 28 templates + 14 `output-small` fixtures, codified with the strip-vs-banner discriminator (→D-046) 2026-06-20 — **Wave-2 Sprint 3 complete**. S4 (DSB) `G-19` positioning-anchor + falsifiable-rationale adopted (→D-050) 2026-06-21 — DSB `DESIGN_SYSTEM.md` now opens with a captured positioning anchor (intake Q1e) + a cite-an-observable rule. `CF-07` source-immutability **adopted-narrowly** (→D-051) 2026-06-21 — only rule 2 (reload-before-write) landed as a CLAUDE.md co-edited-doc rule; rules 1/3/4 parked (held by C-30 / no referent / tensions with the skip-default). **Wave-2 remaining: only `G-18`** (blocked on the unbuilt C-24 oracle prompt) — so Wave 2 is effectively complete bar that one blocked crib; next leverage is **Wave 3** (`/furnace-plan` each). **Wave-3 `CF-29` per-artifact FORMAT file PILOTED on prd-creator (→ [D-052](DECISIONS.md)), 2026-06-21** — authored via `/furnace-plan` as the furnace-trial calibration pass (4 Cowork rounds; cc-subagent missed the must-fixes Cowork caught). New `skills/prd-creator/PRD-FORMAT.md` consolidates the scattered shape-rules; DSB + a scoped context-engineering subset deferred, Rule-of-Two. **Wave-3 `CF-29` propagated to DSB (→ [D-057](DECISIONS.md)), 2026-06-22** — new `skills/design-system-bootstrap/DESIGN_SYSTEM-FORMAT.md` consolidates the `DESIGN_SYSTEM.md` doc-shape rules (doc-only scope; all three shapes; bodies moved from `principles.md`, operative echoes left in the loaded template). Scoped context-engineering subset still deferred, Rule-of-Two. **Wave-3 `CF-29` CLOSED on context-engineering — don't-build, satisfied-by-construction (→ [D-058](DECISIONS.md)), 2026-06-23.** A scope read found CE already meets CF-29 structurally: ~25 artifacts each pair a `templates/*` source with an `examples/output-small` fixture (the C-08 diffable shape contract), and SKILL.md is already a router — no scattered artifact-shape prose to consolidate, and a FORMAT-per-type would be the sprawl CF-29 exists to kill. **CF-29 now complete across all three target skills; Wave-3's lone `implement` crib is done.** **Wave-3 `CF-22` (family router `/which-skill`) DEFERRED (→ [D-059](DECISIONS.md)), 2026-06-23** — planned via `/furnace-plan`; it is [D-014](DECISIONS.md)'s gated fix-candidate B (smart-routing) and no skill-selection failure has been observed, so deferred to a `watching` row (same evidence-gated logic as the CF-29 don't-build). Build when a misfire is logged or the family opens beyond the single author; design preserved in the plan. **Wave-3 `C-07` destructive-regen guard CLOSED don't-build (→ [D-060](DECISIONS.md)), 2026-06-23** — verified against the live `hooks/write-guard.sh`: fail-before-mutate / loud-default-verdict / read-only-validation covered literally by D-005/D-006/D-007; the lineage-hash and rollback-snapshot legs are N/A for a one-shot scaffolder (purpose served by the session-id safety key + git). **Wave-3 `CF-21` wrapper+engine composition CLOSED don't-build (→ [D-061](DECISIONS.md)), 2026-06-23** — premise absent: furnace-plan is one explicit-invoke skill (no variant family), the 3-chain are distinct skills not engine variants, and the lone path-link (`../plan-review/SKILL.md`) is structurally forced (Cowork ZIP skill, not name-invocable). **Wave-3 `C-01` (R/KTD citation graph) + `C-02` (anti-bulk-accept primitives) DEFERRED → watching (→ [D-062](DECISIONS.md)), 2026-06-23** — both real additive mechanisms on the furnace/Cowork loop, but neither failure (dropped requirement; bulk-decline) is observed and both add per-plan ceremony at n=1; build on a logged failure or a 2nd author. **Wave 3 is now COMPLETE — all six cribs resolved with ZERO new builds** (CF-29 propagated/closed across the 3 skills; CF-22/C-01/C-02 deferred; C-07/CF-21 closed don't-build): the expensive `implement`/`integrate`/`(H)` cribs met a repo that had already absorbed the cheaper Wave-1/2 fixes, so they turned out covered-by-construction or premature — which is itself the finding. **Big Rocks `solutions/` scar-tissue library + `CF-05` rule-compression CLOSED don't-build (→ [D-063](DECISIONS.md)), 2026-06-23** — on the 2026-06-09 self-healing-loop council (5/5 build-nothing) + an empirical scan of the 56 retro failure-sections (log mostly healthy; a one-time scan beats a standing library). Lever = periodic aggregation, not a library; a recurring scan ritual is Rule-of-Two-gated. The scan surfaced one hidden shipped-product fix (Pattern A — furnace Check 1 grounds *that* a read happened, not that it grounds the claim), now a `next` board row. Update this marker whenever a source tracker gains cribs or a crib changes status. -->
...
- **~~Build the `solutions/` scar-tissue library~~ — CLOSED don't-build ([D-063](DECISIONS.md#d-063)), 2026-06-23.** Was billed the biggest unbuilt thing. Closed on the 2026-06-09 [self-healing-loop council](council/council-transcript-2026-06-09-self-healing-loop.md) (5/5 build-nothing) + an empirical scan of the 56 retro failure-sections (log mostly healthy; a one-time scan found the lot). The lever is **periodic aggregation, not a library**; a recurring scan ritual is Rule-of-Two-gated (one scan ≠ a standing ritual). Revisit only as a scaffold-emit for high-error project types (rides AB-01's D-009 gate).
- **~~`CF-05` rule-compression method~~ — CLOSED don't-build ([D-063](DECISIONS.md#d-063)), parks with `solutions/`.** The scan showed rule-accretion isn't biting (the repo self-throttles on Rule-of-Two), so compression isn't the lever; detection is.
