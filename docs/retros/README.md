...
- **Never record push / commit / sync state in a retro.** The retro is written *before* the commit/push (above), so any "pushed" / "not pushed as of writing" / "awaiting go" line is a prediction that goes stale the instant the push lands — and `git` is already the source of truth for it. A genuinely pending push is a *handoff* line (gated-on-Rex), not a retro status claim. **Failure it prevents:** a frozen "not pushed" gets relayed verbatim by the next `/session-start` as if current — observed three times (the 2026-06-20 lifecycle near-miss; 2026-06-21 relaying "awaiting your go to push" for commits already on the remote; 2026-06-22 a CF-29-DSB retro carrying "not yet pushed" for work already on `origin/main`; n=3). *(At n=3 the previously-declined backstop was **adopted** (2026-06-23): `/session-start` now runs `git status -sb` / HEAD-vs-origin and reports git's truth, ignoring any retro push/sync line. Source-removal alone lost three times — the deterministic check fires every session regardless of author slip; the don't-write-it rule above stays as the first line of defense.)*
