...
- Updated [`BACKLOG.md`](../BACKLOG.md) to fold in both pilots, the cross-pilot design, Rex's decisions, and a sharpened promotion trigger.
...
- Brownfield audit is **parked, not active** — no next-session pickup. It surfaces again only when real drift bites a project; then build `/audit-context` per the profile-gated shape in the reports/BACKLOG.
