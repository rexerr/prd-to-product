...
**Contract-test layer + judge calibration (CE brainstorm mine, 2026-07-01)** — [mined doc CEB-1/2/3](../docs/mined/2026-07-01-compound-engineering-brainstorms.md). Adds the *cheap deterministic half* the plan lacked and sharpens the judge:

- **Assert the string, not just the behavior — verified shipped.** Every's repo ships contract tests that string-assert skill *prose* holds (`tests/pipeline-review-contract.test.ts`, `review-skill-contract.test.ts`, `unified-plan-artifact-contract.test.ts`): e.g. a checklist must literally say "Testing addressed", not "Tests pass". This is a free, deterministic first gate *below* the expensive behavior eval — catch prose drift with a string assertion; reserve the trial-based measurement for whether the wording changes behavior. Our substitution-diff machinery is the analog; a prose-invariant contract test is the extension.
- **A gradeable framing bar with a worked exemplar pair** — "lead with observable behavior, explain why the fix works, cite an in-repo pattern, 2-4 sentences," shipped with a weak/strong example. A concrete rubric target for a prose eval (not "be clear").
- **Judge calibration beyond ≥3-trials:** fixed `sample_seed` across conditions (compare the same items, not re-sampled noise); **anchor samples** — a fixed known-score set in every judge batch, recalibrate if anchors drift >0.5 (catches judge drift the current plan doesn't guard); rubric-change invalidates prior comparisons → re-baseline. From the shipped `ce-optimize` (`skills/ce-optimize/SKILL.md`).
