...
### Reload a co-edited file before writing it

Before writing a file another agent or process may have changed since you last read it — the Cowork ledger above, a doc open in Cowork, anything a background agent touches — reload it from disk first and write from that, never from a stale in-context buffer. Within a single session the Edit tool already enforces this (it rejects a write to a file that changed since your Read); this rule carries the discipline to the cross-agent case the harness can't see. **Failure it prevents:** clobbering an accreted or collaborator-made edit by overwriting from a copy that predates it — the [D-018](docs/DECISIONS.md) ledger sweep is the one instance already named; this is the general rule it is a case of. (Adopted from crib CF-07 rule 2, [D-051](docs/DECISIONS.md); CF-07's three other rules were parked — see that decision.)
