...
| Cross-session commit-sweep guard | watching | — | n=1 (2026-06-24 2nd session): a concurrent Claude-Code session's broad `git add` swept this session's uncommitted `D-065` hunk into its own commit `d3c006d`. On a 2nd sweep, add an explicit-path-staging rule to `CLAUDE.md` ("never `git add -A`/`commit -a`; stage named paths") — agent-config edit, gated. Rule of Two, n=1 — don't accrete yet | [retro](docs/retros/2026-06-24-furnace-blind-review-hardening.md) |
