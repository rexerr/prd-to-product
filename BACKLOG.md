...
| Port explicit-path-staging rule to CE scaffold | backlog | — | D-066's "stage named paths, never `git add -A`/`commit -a`" git-hygiene rule is generic — fold into the context-engineering session-discipline/code-rules template (+ `output-small` fixture) on the next CE template session | [D-066](docs/DECISIONS.md) |
