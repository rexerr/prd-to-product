...
| One-board model → context-engineering scaffold | watching | — | D-009 council done (2026-06-24): **reject day-one port; the model is a graduation TARGET, not a born-with default.** Re-enters on a 2nd scaffolded project hitting backlog bloat / reinventing lanes (Rule-of-Two) → repoint the graduation rule's target at named lanes (no day-one coupling). Council flagged the median-user + does-graduation-ever-fire premises as unverified. **Provisional — a `/devils-advocate` re-test is pending** (Rex-as-actual-user may flip the median-user premise); no D-NNN minted until that settles | [council](docs/council/council-transcript-2026-06-24-batch-b.html), [D-054](docs/DECISIONS.md#d-054) |
