...
| Generator skips seeded allowlist when hooks are off | fix | backlog | — | area:generator | A no-hooks scaffolded project loses the seeded read-only permissions allowlist — make the generator emit a permissions-only `settings.json` when hooks are off. | [retro](docs/retros/2026-06-08-agent-process-groups-2-3-4.md), [decisions](skills/context-engineering/generator/decisions.md) |
