...
|---|---|---|---|---|
| Agent-process & context-harness upgrades | next | 1 | Residual: the group-5 in-repo dogfood self-edit loose end | [brief](docs/agent-process-brief.md) |
| Invariant/semantic output checks | next | 2 | OPEN: provenance-grounded check only (write-guard + no-jargon-leak both done — [D-064](docs/DECISIONS.md#d-064)) | [council](docs/council/council-report-2026-06-08.html) |
...
| One-board model → context-engineering scaffold | watching | — | [D-068](docs/DECISIONS.md#d-068) (council + devil's-advocate, 2026-06-24): **not a scaffold day-one default** — stays a graduation-TARGET candidate. Re-enters on a 2nd scaffolded project hitting backlog bloat / reinventing lanes (Rule-of-Two) → repoint the graduation rule's target at named lanes (no day-one coupling). Rex's "see a kanban" need is met by the local rendered view (`BACKLOG.html`), not a product change | [council](docs/council/council-report-2026-06-24-batch-b.html), [D-068](docs/DECISIONS.md#d-068) |
...
| Port a backlog→board render tool to the scaffold | backlog | — | Rex wants the kanban **view** (not the parked model — [D-068](docs/DECISIONS.md#d-068)) in new projects. `scripts/render-backlog-kanban.py` is coupled to this repo's lane-table shape; the scaffold ships the multi-section shape, so build a **generic** renderer that maps the scaffold's default `BACKLOG.md` sections → columns and emit it as an optional dev-tool (opt-in, process-free, like the scaffolded `check-live-links`). Softer yes than row 40 — a view forces no day-one ceremony, so the council's objections don't apply | [D-068](docs/DECISIONS.md#d-068) |
