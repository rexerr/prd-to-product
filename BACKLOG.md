...
|---|---|---|---|---|---|---|
| [Project-setup system — review & design](docs/handoffs/project-setup-system-handoff.md) | research | next | 1 | area:skill | The trio is necessary-not-sufficient: map the full idea→set-up-right→built→secure/tested path; next session reviews real projects + TanStack template/workflow/devtools + online research, then decides compose-vs-build. | [handoff](docs/handoffs/project-setup-system-handoff.md) |
| /mine compound-engineering-plugin (remainder) | research | next | 2 | area:skill-templates | The authoring-discipline slice is mined (AGENTS.md + skill-design solutions + 3 large skills); the remainder is not — 24 more skills, 19 more solutions docs (best-practices/integrations/workflow), the eval harnesses, the cross-host converter. Every skipped source has proven worthy on return — mine the rest. | [slice mine](docs/mined/2026-07-01-compound-engineering-plugin.md) |
| Skill-craft consolidation pass | feature | next | 3 | area:skill-templates | One dedicated session folding the five parked skill-craft buckets (intent, gstack, devtools, Every-guides, CE-plugin) into the skills + scaffold templates — their rule-of-2 gates have tripped across four independent sources — and adjudicating two forks: the ≤500-line cap (enforce vs. conditionality discipline) and deletion-test vs. mandatory failure-mode clauses. Run after the remainder mine so inputs are complete. | [mine](docs/mined/2026-07-01-compound-engineering-plugin.md) |
...
| Chain auto-compose | feature | watching | — | area:skill | An orchestrator that chains the skills together automatically — only if hand-offs start getting dropped. Strong external prior art: TanStack/workflow's durable append-only-log + derived-state + approval-pause + idempotent-resume model — adopt the execution model (not the TS runtime) when picked up. | [D-014](docs/DECISIONS.md#d-014), [mine](docs/mined/2026-07-01-tanstack-workflow.md) |
...
| Docs-craft patterns from workflow mine | chore | watching | — | gate:rule-of-2, area:docs | Three docs borrows: generated-reference + a thin hand-written orientation layer over it, a `research/`-style status-column disposition table for our council/brainstorm/mined archives, and a self-epistemic comparison-doc type (tag each row differentiator/table-stakes/non-goal; mark partial when unproven). Fold in on the next docs/template edit. | [mine](docs/mined/2026-07-01-tanstack-workflow.md) |
| Strategy layer upstream of PRD (prd-creator) | feature | watching | — | gate:rule-of-2, area:prd-creator | A Rumelt-style strategy doc (target problem / approach / persona / metrics / tracks / "not working on") upstream of the PRD, re-interviewed periodically — the tracks + anti-scope sections are the delta over the current interview; verified against Every's ce-strategy. Fold on the next prd-creator edit or promote when a real project needs it. | [mine](docs/mined/2026-07-01-every-agent-native-guides.md) |
...
| Skill-authoring upgrades from intent mine | chore | watching | — | gate:rule-of-2, area:skill-templates | Three parked skill-craft borrows: richer SKILL.md frontmatter validation (spec-key/name/length/line), the Plausible·Silent·Grounded bar for a cited failure mode, and a cross-model authoring checklist — fold in on the next skill/template edit or when a new skill is authored. | [mine](docs/mined/2026-06-30-tanstack-intent.md) |
...
| Chain-handoff audit vs. CE incident rules | fix | backlog | — | area:skill | Audit our chain skills' hand-offs against four incident-grounded rules from the CE-plugin mine: always-fire routing inline (never only in a reference — the paraphrase suppresses the load, issue #714), name the invocation primitive ("Call /X" reads as "tell the user to type /X"), no instruction-files named on the read path, information-asymmetric load stubs on references. | [mine](docs/mined/2026-07-01-compound-engineering-plugin.md) |
