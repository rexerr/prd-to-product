...
- **Validation finding: skill chain doesn't auto-compose; mental model mismatch.** First observation: 2026-05-11 Squirreled validation. User invoked `/context-engineering` with a working brief, expecting the chain (prd-creator → context-engineering → design-system-bootstrap) to run as one orchestrated flow. The current architecture has three separate skills that hand off manually — composition is the user's responsibility. Result: user skipped prd-creator entirely. context-engineering gracefully accepted the brief in place of a PRD, but the resulting PRD will be thinner than a prd-creator interview would produce. Sophisticated user mental model says "the repo is called prd-to-product, so there should be one command that runs the chain." Three possible fixes: **(A) build a `/prd-to-product` meta-skill** that orchestrates; **(B) smart routing in context-engineering cluster 0** — detect brief-vs-PRD and offer to call prd-creator first; **(C) better cluster-0 question + README** — text changes only. Recommend C as cheapest first move, escalate to B if C doesn't solve. **Defer the fix until Squirreled validation completes** — major finding worth its own iteration session.
