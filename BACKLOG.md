...
- **Validation finding: prd-creator cluster 0 over-structures when source material is dense.** First observation: 2026-05-11 Squirreled validation. The intake.md prescribes a single open free-text question for cluster 0 ("do you have a working brief, research dump, transcript, message thread, or notes you want me to read first"). The skill instead split into Y/N sub-questions (0a, 0b), surfaced internal jargon ("Cluster 0," "as source," "extract cold"), and tried to triage every reference mentioned inside the brief (build-defaults pilot, neighbor product ideas) as separate decisions. The model is being "helpful" by over-structuring. Likely fix: ~10–15 lines in `skills/prd-creator/generator/intake.md` — add an explicit anti-pattern note ("do not split into sub-questions; absorb brief contents silently; let cluster 1+ surface what matters") and an example of the question being asked correctly. **Defer the fix until the Squirreled validation completes** — don't change the skill mid-experiment. Aggregate with any other onboarding friction the validation surfaces.
