...
|---|---|---|---|---|
| Pattern-A furnace grounding fix (Check 1b) | next | 1 | Furnace Check 1 verifies a read *happened*, not that it was broad/granular/forward enough to ground the claim (n=9+, recurs after 1a — surfaced by the [D-063](docs/DECISIONS.md#d-063) retro-scan). Add a Check-1 sub-case + scaffold the same in the emitted verification rule (both shapes + `output-small`). `/furnace-plan` candidate (shipped product) | [D-063](docs/DECISIONS.md#d-063) |
