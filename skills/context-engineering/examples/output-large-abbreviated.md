...
**Provenance banner (D-046):** every emitted file in this tree leads with the `<!-- Generated by the context-engineering skill … -->` banner — directly after the closing `---` for the frontmatter rules (`ai-shared.md`, the `ai-<surface>.md` stubs), after the shebang for hooks, the `#` form at the top of `.codex/config.toml`, and via the top `"//"` key for `settings.json`. Omitted from the per-file sketch below for brevity.
