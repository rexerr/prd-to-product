...
**The `docs/tickets/` graduation layer is runtime-created, not generator-written.** The scaffolded `BACKLOG.md` ships thin and lean; the session-discipline graduation rule + the `/end-session` retirement ritual tell a project to split it into a thin index + on-demand `docs/tickets/<slug>.md` parts (archived to `docs/tickets/archive/`) *only when it outgrows the session-start read*. The generator never emits a `tickets/` folder at scaffold time — the project creates it at graduation. `docs/tickets/` sits under `docs/`, so the allowlist above already admits it (no amendment). There is no inclusion-table row for it.
