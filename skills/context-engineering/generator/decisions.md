...
2. **User's first work** — from Q31 (`phase_user_*`). Split `phase_user_task_placeholder` into one row per discrete task — one row per comma-clause is the default (a "deploy" sub-task folds into the deploy-shell row above, not its own row). `Seq` continuing 2, 3, …, Type `feature` (or `chore`/`fix` as fits), Lane `next`. Give each row **one** `area:` (the task's primary subsystem — the max-2 tag budget is an `area:` *plus* an optional `gate:`, not two areas), plus `gate:visual-confirm` when it changes what renders in *this app's* UI and the visual gate is on. `Gloss` = the task in one plain line.
3. **Deferred items** — from `out_of_scope_list` / `deferred_capabilities_list_or_none`: emit only genuine *future* work as `icebox` rows (`Seq —`, one `area:`, gloss naming what would un-park it). The cut: an item phrased as an **upgrade path** ("stronger X *if* Y proves insufficient", "X beyond Y") is future work → `icebox`; a flat **exclusion** ("no X", "not doing Y") is a hard boundary → leave it in the PRD, no row.
...
## Provenance-grounded scan before finalizing

At the same pre-write gate as the scaffolding-leak scan above — **before writing the scaffolded files** — confirm that every composed project fact traces to something the user actually gave at intake. **Unlike** the leak scan, this one is **not** a grep: "is this fact invented?" has no pattern. It is a **bounded semantic check** (judgment), and what keeps it trustworthy is the *scoping below*, not a regex — a "check every sentence" pass would false-positive on legitimate phrasing variation and train the user to ignore it.

**Scope — these composed-fact slots only** (the slots the generator *composes from free intake*, where a fabrication can enter):

- `board_seed_rows` (each board row's item + gloss), plus the composed content fills: `product_summary_paragraph`, `target_users_list`, `core_problem_paragraph`, `main_workflow_steps`, `primary_data_flow_name`, `primary_data_flow_steps`, `out_of_scope_list`, `additional_stack_summary`, `data_persistence_paragraph`, `external_integrations_list_or_none`, `folder_structure_summary`, `architecture_rules_numbered_list`.
- **Out of scope — do not check:** 1:1 substitutions copied verbatim (`project_name`, repo paths, the `stack`/`deploy_target` enums) and **derived** values computed deterministically from them (`stack_summary_one_line`). These can't be fabricated — they're copied or computed.

**The trace test.** For each project-specific factual claim in those slots, confirm it maps to a named intake answer (a captured field from the State map above, or one of the interview content fills). A claim with **no** intake source is fabricated → fix before writing: remove it, or correct it to what the intake supports. **Enum-derived / gist-table content inside a composed slot still counts as grounded** — e.g. a deploy-shell board-row gloss built from the deploy-step gist table keyed on the `deploy_target` enum traces to a named intake answer, so do **not** flag it. The target is *free-text claims with no intake source*, never deterministic content that merely lives in a composed slot.

**Never flag scaffold structure.** Lanes, `Seq`, the generic process/boilerplate rows, the "Generated by…" banner, and rule-template wording are the scaffold's fixed shape, not project-fact claims — always exempt. Only project-specific factual *content* is in scope.

This is the mechanical backstop to the **Board seeding** rule (board rows are "composed from the intake answers … not from a fixed stack table") and to the "scaffolds shape, not content" invariant: the prose prevents a fabricated fact being written; this scan catches it if the rule is forgotten on any one run — the same prevent/catch split as the leak scan. It is a generator **self-check** (it grades content the generator just composed), kept narrow and lookup-like to limit the grading-its-own-work weakness; the `output-small` regression fixture and the user's own confirmation remain the independent layer.

**Failure it prevents:** the generator **composing a project fact the intake never gave** — an invented board row, a constraint, a data-flow step, a `lib/` path, or a stack detail — the fabricated-provenance class this check exists to catch (the provenance-grounded sibling of the scaffolding-leak scan above). Caught before it reaches the user's repo and propagates.
