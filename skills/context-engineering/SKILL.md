...
- The leading `<!-- Template: … -->` **authoring block** (its body has `Template:` / `Purpose:` / `Use this template when:` lines) is template-only and **stripped at emit** — it never reaches the generated file.
- The `<!-- Generated by the context-engineering skill … -->` **provenance banner** sits immediately after that authoring block and **is emitted verbatim** as the file's leading line. Every emitted file carries it; see `generator/decisions.md` "Generated-file provenance banner" for the per-format syntax and the strip-vs-banner discriminator.
