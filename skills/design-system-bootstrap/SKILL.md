...
- **Re-running must not clobber hand-edited files.** This skill writes a token file, three seed components with paired CSS modules, and `DESIGN_SYSTEM.md` — all governed by the **non-destructive write guard** (`generator/decisions.md`): before writing any file that already exists, show a diff and ask **overwrite / skip** (default skip; merge only for the rule file and Tailwind config). It is **enforced** by the global `write-guard.sh` hook ([`hooks/README.md`](../../hooks/README.md)) when installed and the run is armed (interactive → `ask` dialog, headless → `deny`; arm at run start per `generator/decisions.md`) — but still honor the prose, since the hook may be absent or bypassed. The qventus near-miss (a hand-authored `tokens.css`/design system) is exactly the case it protects.
