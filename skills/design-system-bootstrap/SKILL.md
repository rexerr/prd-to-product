...
- **Re-running must not clobber hand-edited files.** This skill writes a token file, three seed components with paired CSS modules, and `DESIGN_SYSTEM.md` — all governed by the **non-destructive write guard** (`generator/decisions.md`): before writing any file that already exists, show a diff and ask **overwrite / skip** (default skip; merge only for the rule file and Tailwind config). This is a *prose* guard the agent must honor — it is **not yet hook-enforced**, so do not drop it under context pressure. The qventus near-miss (a hand-authored `tokens.css`/design system) is exactly the case it protects.
