...
4. **The semantic-leak scan flagged something.** When the pre-write scan (`decisions.md` "Semantic-leak scan before finalizing") flagged a possible leak, note the *resolution*, not a live warning — by summary time the user has already kept or removed it. One line per flag: "Flagged <X> as a possible leak; you chose to keep / remove it." Omit entirely if nothing was flagged.
