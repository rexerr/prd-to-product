...
1. Ask the cluster's questions. Up to three per call. Free-text fills ask one at a time. Branching questions offer two to four options, never more. Three rules govern the *form* of every question:
   - **Open the choice before you ask it (explainer-before-choice).** A branching or decision question opens with what it is, why it's needed, and *what changes if you pick differently* — one at a time, never a menu dumped at once. **Failure it prevents:** a user who does not know the terms is handed a decision with no way to weigh it.
   - **Expose your stance where you have one, scoped (recommended-answer).** When cluster-0 source material covers a question, present a draft answer drawn from it for confirm/edit (the full rule is in cluster 0); ask cold only where the material is thin. On a branching/decision question, name a recommended option *among those already presented*, marked as a default to edit — **never originate a product direction the user has not chosen** (the skill does not invent decisions). Genuinely open free-text questions ("what is this product," "who is the V1 user," "what are you doing instead") stay **bare** — do not fabricate an example to react to, which leads the user and risks inventing. Note the distinction: "ask cold" means *no material-derived draft*, not "pose a bare question"; a bare open question is legitimate.
   - **Don't re-ask what the material answers; force its contradictions (explore-before-you-ask).** If cluster-0 material already answers a question, draft from it rather than spend a turn. When a later cluster's answer contradicts the cluster-0 brief, surface it as a *single forced-choice question* ("the brief says X, you just said Y — which holds for V1?") rather than absorbing the conflict silently.
...
Wait for explicit user confirmation before writing files. If `docs/PRD.md` or `docs/BRAND.md` already exists, flag it here — it will be shown as a diff for overwrite/skip consent at write time (default skip), never silently replaced (per `decisions.md` "Non-destructive write guard"). Run the **semantic-leak scan** (`decisions.md` "Semantic-leak scan before finalizing") over the about-to-be-written content and surface any hit for the user to keep/reword/cut before writing. Then run `decisions.md` to determine final emission and write the PRD, applying the write guard to every target.
