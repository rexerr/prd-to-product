...
**1b. Sub-case — ground each claim at the breadth, granularity, and forward-reach it asserts.**
A read that *happened* is not automatically a read that *grounds the claim*. Match the read to the claim's scope: a claim about "all N" needs a read across all N (breadth); a claim about a specific line or value needs that line read, not the file skimmed (granularity); a claim about what a change will *cause* downstream needs the callers/consumers read, not just the edit site (forward-reach). If the claim is broader than the read behind it, widen the read or narrow the claim.
*Failure it prevents:* Check 1 is satisfied — a read did happen — but it was too narrow, too shallow, or too backward-looking to support the claim resting on it (n=9+, recurring after the 1a tightening; hidden because each instance looked like a different micro-type — granularity, forward-inference, scope-breadth, negative-claim — so no single shape tripped Rule-of-Two).
